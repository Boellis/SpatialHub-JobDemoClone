...
SUPPORTED_OVERRIDES = (
    ...
    "biomass_water", "crop_area", "food_store",
)


def build_config(overrides, crew_size=15):
    """Build a crewed config from the defensible base with overrides applied.
    Returns (xml, applied, ignored) where applied/ignored are override-name lists."""
    xml = DEFENSIBLE_CONFIG.read_text()
...
def run_playground(biosim_url, overrides, mode="passive", difficulty="off",
    ...
    `baseline` is the LLM/RL-coupled defensible config with NO overrides, run the
    same way -- the comparison point for the farmer's tweaks.
    """
    xml, applied, ignored = build_config(overrides, crew_size)
    result = run_config(biosim_url, xml, mode, cap, difficulty, crew_size)

    base_xml, _, _ = build_config({}, crew_size)
